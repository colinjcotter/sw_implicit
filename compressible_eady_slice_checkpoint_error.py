...
parser.add_argument('--ny', type=int, default=1, help='Number of cells in y. Default 1.')
...
parser.add_argument('--tmax', type=float, default=0.1, help='Final time in days. Default 35.')
...
m = fd.PeriodicRectangleMesh(base_columns, ny=args.ny, Lx=2*L, Ly=1.0e-5*L,
                             direction="both",
                             quadrilateral=True,
...
theta_back = fd.Function(Vt, name="theta_back").assign(thetan)
...
hydrostatic_rho(Vv, V2, mesh, thetan, rhon=rhon, pi_boundary=fd.Constant(1),
                    ...
                    top=False, Pi=Pi_ref)
rho_back = fd.Function(V2, name="rho_back").assign(rhon)
...
with fd.CheckpointFile(name+".h5", 'w') as checkpoint:
    checkpoint.save_mesh(mesh)
    checkpoint.save_function(theta_back, idx=idx,
                             timestepping_info={"time": t, "tdump": tdump, "tcheck": tcheck})
    checkpoint.save_function(rho_back) 
    PETSc.Sys.Print("Checkpointed at t = ", t, "idx = ", idx)

# print out checkpoint
with fd.CheckpointFile(name+".h5", 'r') as checkpoint:
    mesh = checkpoint.load_mesh("mesh")
    timestepping_history = checkpoint.get_timestepping_history(mesh, name="theta_back")
